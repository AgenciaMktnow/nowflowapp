...
export default function OrgDetailsModal({ orgId, onClose }: OrgDetailsModalProps) {
    ...
    const handleDeleteOrganization = async () => {
        if (!orgId || !details) return;

        setIsDeleting(true);
        const toastId = toast.loading('Iniciando limpeza profunda (Storage + DB)...');

        try {
            const { data, error, fileCount } = await adminService.deleteOrganizationWithStorageCleanup(orgId);

            if (error) {
                toast.error(`Falha na exclusão: ${error.message}`, { id: toastId });
            } else {
                toast.success(
                    `Organização excluída! ${fileCount} arquivos removidos e ${data.total_records} registros deletados.`,
                                        ...
                                        <div className="mt-12 p-6 rounded-xl border border-red-500/20 bg-red-500/5">
                                            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-text-subtle mt-1 max-w-xl">
                                                        Esta operação apaga todos os arquivos, membros, projetos, tarefas e a própria organização permanentemente. Não há "desfazer".
                                                    </p>
                                                </div>
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    className="px-6 py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-lg transition-all shadow-lg shadow-red-600/20 flex items-center gap-2"
                                                >
            ...
            {/* DELETE CONFIRMATION MODAL */}
            {
                isDeleteDialogOpen && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
                        <div className="w-full max-w-md bg-[#121214] border border-red-500/30 rounded-2xl p-8 shadow-[0_0_50px_rgba(239,68,68,0.2)]">
                            ...
                            {deleteConfirmStage === 1 ? (
                                <div className="text-center">
                                    ...
                                    <h3 className="text-xl font-bold text-white mb-2">Atenção Crítica!</h3>
                                    <p className="text-text-subtle text-sm leading-relaxed mb-8">
                                        Você está prestes a apagar **{(details?.org_name || 'Agência').toUpperCase()}**. <br />
                                        Isso removerá **{details?.db_rows || '?'}** registros do banco e todos os arquivos do storage.
                                    </p>
