            ...
            if (error) {
                const fb = await supabase.from('tasks').select(`
                    *,
                    ...
                    assignee:users!tasks_assignee_id_fkey(full_name, avatar_url),
                    creator:users!tasks_created_by_fkey(id, full_name)
                `).neq('status', 'DONE').order('priority', { ascending: false });
                setTasks(fb.data || []);
            } else {
                setTasks(data || []);
            }
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-background-dark">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                <div className="max-w-7xl mx-auto p-6 flex flex-col gap-6">
                    ...
                    {/* Action Bar - Filters + Nova Tarefa */}
                    <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                        {/* Filter Tabs */}
                        <div className="bg-surface-highlight/50 p-1.5 rounded-xl backdrop-blur-sm border border-white/5 flex items-center gap-1">
                            {(['MINE', 'CREATED', 'REVIEW'] as const).map(tab => (
                                <button
                                    key={tab}
                                    onClick={() => setActiveTab(tab)}
                                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === tab ? 'bg-primary text-background-dark shadow-[0_0_15px_rgba(19,236,91,0.2)]' : 'text-text-secondary hover:text-white hover:bg-white/5'}`}
                                >
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-[#0fd650] text-background-dark font-bold rounded-xl transition-all shadow-[0_4px_15px_rgba(19,236,91,0.3)] hover:scale-105 active:scale-95"
                        >
                    ...
                    {/* Toolbar - Status Filter, Counter, Sort */}
